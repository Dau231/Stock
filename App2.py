...
#                 end_date_obj = pd.to_datetime(end_date) # Convert end_date to datetime object
#                 future_dates = [end_date_obj + timedelta(days=i) for i in range(1, 8)]
# # Create DataFrame for output
#                # Create DataFrame for output - Modified for cleaner output
#                 forecast_df = pd.DataFrame({"Date": future_dates, "Predicted Price": predicted_prices[1:]})
#                 forecast_df = forecast_df[["Date", "Predicted Price"]]  # Select only Date and Predicted Price
...
# Convert dates to strings for display (important for Streamlit)
                # forecast_df['Date'] = forecast_df['Date'].dt.strftime('%Y-%m-%d')

                # st.dataframe(forecast_df,  hide_index=True) # Hide index and display as a dataframe
# Create future dates - Corrected to start from the day AFTER the specified end date
